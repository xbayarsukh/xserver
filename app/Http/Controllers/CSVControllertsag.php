...
class CSVController extends Controller
{
    ...
    private $ugluuniiAjilDuusahTsag;
    private $udiinTsaiEhleh;
    private $udiinTsaiDuusah;
    private $ajilDuusahTsag;
    private $ilvvTsag1Ehleh;
    private $zawsarlagaa;
    private $hagasUdur;
    private $hoyrZuunGuch;
    ...
    // dd($calculations, $key, $filteredValue);
    ...
    {
        $ajilEhleh = $this->getCalculationValue($calculations, '1');//8:30
        $ilvvTsag1Ehleh = $this->getCalculationValue($calculations, '2');//17:40
        $ilvvTsag1Duusah=$this->getCalculationValue($calculations,'3');//06:00
        $ugluuniiIlvvTsagEhleh=$this->getCalculationValue($calculations,'4'); //'06:10'

        $ilvvTsag2Ehleh=$this->getCalculationValue($calculations, '5'); //22:00
        $ilvvTsag2Duusah=$this->getCalculationValue($calculations, '6'); //06:00

        $ajilDuusahTsag=$this->getCalculationValue($calculations,'7');//17:30
        $ugluu=$this->getCalculationValue($calculations,'8');//07:00

        $ugluuniiAjilDuusahTsag=$this->getCalculationValue($calculations,'9');//11:50

        $udiinTsaiEhleh=$this->getCalculationValue($calculations,'10');//12:00
        $udiinTsaiDuusah=$this->getCalculationValue($calculations,'11');//13:00
        $hagasUdur=$this->getCalculationValue($calculations,'12');//12:30
        $zawsarlagaa=$this->getCalculationValue($calculations,'13');//10min
        $hoyrZuunGuch=$this->getCalculationValue($calculations,'14');//230 min
        ...
        foreach ($arrivalRecords as $date => $dailyRecords) {
            ...
            foreach ($dailyRecords as $arrivalRecord) {
                ...
                if ($startTime > $ajilEhleh && !in_array($date, $halfDayDates)) {
                    $lateArrivalSeconds += Carbon::parse($startTime)->diffInSeconds(Carbon::parse($ajilEhleh));
                ...
                if ($startTime && $endTime) {
                    ...
                    $overtimeStartB = Carbon::parse($ilvvTsag1Ehleh);
                    $overtimeEndB = Carbon::parse($ilvvTsag1Duusah)->addDay();
                    ...
                    $morningOvertimeStart = Carbon::parse($ugluuniiIlvvTsagEhleh);
                    $morningOvertimeEnd = Carbon::parse($ajilEhleh);
                    ...
                    $overtimeStartC = Carbon::parse($ilvvTsag2Ehleh);
                    $overtimeEndC = Carbon::parse($ilvvTsag2Duusah)->addDay();
                    ...
                    if ($startTimeCarbon > Carbon::parse($ajilEhleh) && !in_array($date, $halfDayDates)) {
                        $lateArrivalSeconds += $startTimeCarbon->diffInSeconds(Carbon::parse($ajilEhleh));
                        $countLate++;
                    }

                    if ($endTimeCarbon < Carbon::parse($ajilDuusahTsag) && !in_array($date, $halfDayDates)) {
                        $earlyLeave++;
                    }

                    if ($startTimeCarbon->between(Carbon::parse($ugluu), Carbon::parse($ajilDuusahTsag), true) || $endTimeCarbon->between(Carbon::parse($ugluu), Carbon::parse($ajilDuusahTsag), true)) {
                        $workedTimeInSeconds = $this->calculateWorkedTime($startTime, $endTime, $ugluuniiAjilDuusahTsag, $udiinTsaiEhleh, $udiinTsaiDuusah, $ajilDuusahTsag, $ilvvTsag1Ehleh, $zawsarlagaa, $hagasUdur, $hoyrZuunGuch);
    ...
    private function calculateWorkedTime($startTime, $endTime, $ugluuniiAjilDuusahTsag, $udiinTsaiEhleh, $udiinTsaiDuusah, $ajilDuusahTsag, $ilvvTsag1Ehleh, $zawsarlagaa, $hagasUdur, $hoyrZuunGuch)
    {
        ...
        $IQ6 = $this->timeToMinutes($ugluuniiAjilDuusahTsag);
        $IQ7 = $this->timeToMinutes($udiinTsaiEhleh);
        $IQ8 = $this->timeToMinutes($udiinTsaiDuusah);
        $IQ10 = $this->timeToMinutes($ajilDuusahTsag);
        $IQ11 = $this->timeToMinutes($ilvvTsag1Ehleh);
        $IQ12 = $zawsarlagaa;
        $IQ17 = $this->timeToMinutes($hagasUdur);
        $IQ20 = $hoyrZuunGuch;

        $departureMinutes = $this->timeToMinutes($endTime);
        $arrivalMinutes = $this->timeToMinutes($startTime);
        $lunchTimeStartMinutes = $this->timeToMinutes($udiinTsaiEhleh);
        $lunchTimeEndMinutes = $this->timeToMinutes($udiinTsaiDuusah);
