    ...
    <div class="bg-gray-100 shadow-sm min-h-screen">
        <div class="container mx-auto px-4">
            ...
            <div class="shadow overflow-hidden rounded border-b border-gray-200 bg-white mt-10">
                ...
                <h1 class="text-2xl font-bold mb-4 text-left py-4 px-2">PDFCompany一覧</h1>

                <div class="flex flex-wrap gap-2 mb-4 px-2">
                    <x-button purpose="search" href="{{ url('/pdfCompany/create') }}">
                        新規登録
                    </x-button>
                    ...
                    <div class="table-responsive mt-10">
                        <table class="border-collapse border border-slate-400 min-w-full bg-white mt-5">
                            ...
                            <tbody>
                                @foreach ($pdfcompany as $index =>$record)
                                <tr class="border-b border-gray-200 hover:bg-blue-50">
                                    <td class="border border-slate-300 px-4 py-2" data-label="番号">
                                        {{ $index +1 }}
                                    </td>
                                    <td class="border border-slate-300 px-4 py-2" data-label="会社名">
                                        {{ $record->name }}
                                    </td>
                                    <td class="border border-slate-300 px-4 py-2" data-label="動作">
                                        <div class="flex space-x-2"> <!-- Use flex and space-x-2 to align buttons horizontally -->
                                            <a href="{{ route('pdfCompany.edit', $record->id) }}" class="p-2 hover:bg-yellow-200 inline-block">
                                                <img src="{{ asset('2.svg') }}" alt="編集" class="w-10 h-10">
                                            </a>
                                            <form action="{{ route('pdfCompany.destroy', $record) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                            ...
                                            <a href="{{ route('pdfCompany.show', $record->id) }}" class="p-2 hover:bg-green-200 inline-block">
