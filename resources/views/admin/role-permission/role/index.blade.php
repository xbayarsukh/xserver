...
<div class="bg-gray-100 shadow-sm min-h-screen">
    ...
    <div class="shadow overflow-hidden rounded border-b border-gray-200 bg-white mt-10">
        ...
        <h1 class="text-2xl font-bold mb-4 text-left py-4 px-2">権限一覧</h1>



        <div class="flex flex-wrap gap-2 mb-4 px-2">
            <x-button purpose="search" href="{{ url('/admin/role-permission/roles/create') }}">
                新規登録
            </x-button>
        </div>
    </div>
            ...
            <table class="border-collapse border border-slate-400 min-w-full bg-white mt-5">
                <thead class="bg-gray-200 text-black">
                    <tr>
                            ...
                            <th class="border border-slate-300 text-left py-3 px-4 uppercase font-semibold text-sm">番号</th>

                            <th class="border border-slate-300 text-left py-3 px-4 uppercase font-semibold text-sm">名前</th>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr class="border-b border-gray-200 hover:bg-blue-50">
                            <td class="border border-slate-300 px-4 py-2">{{ $role->id}}</td>


                            <td class="border border-slate-300 px-4 py-2">{{ $role->name ?? ''}}</td>


                            <td class="border border-slate-300 px-4 py-2">
                                <a href="{{ url('/admin/role-permission/roles/'.$role->id.'/give-permissions') }}"   class="p-2 hover:bg-stone-200 inline-flex items-center justify-center">
                                    <img src="{{ asset('3.svg') }}" alt="Add" class="w-10 h-10">
                                </a>
                                <a href="{{ url('/admin/role-permission/roles/'.$role->id.'/edit') }}"   class="p-2 hover:bg-yellow-200 inline-flex items-center justify-center">
                                    <img src="{{ asset('2.svg') }}" alt="編集" class="w-10 h-10">
                                </a>

                                <form action="{{ url('/admin/role-permission/roles/'.$role->id.'/delete') }}" method="POST" class="inline-flex">
                                    @csrf
                                    @method('DELETE')
